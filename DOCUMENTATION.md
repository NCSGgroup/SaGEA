...
### 5.4 Complete Error Analysis Example

The following example computes **Error-II** (between-product discrepancy) for CSR, GFZ, and JPL RL06 solutions
using both TCH and TCA, starting from the raw `.gfc` files:
...
# ── 1. Paths to three processing centers ──────────────────────────────────────
dir_csr = pathlib.Path("/data/CSR/RL06")
dir_gfz = pathlib.Path("/data/GFZ/RL06")
dir_jpl = pathlib.Path("/data/JPL/RL06")
...
dirs = [dir_csr, dir_gfz, dir_jpl]
paths = [sorted(d.rglob("GSM-2*0600")) for d in dirs]

shc_gif48 = sagea.SHC.io.from_gfc(path_gif48, lmax=lmax)

# ── 2. Post-process each solution identically ─────────────────────────────────
grid_list = []
for path_list in paths:
    dates_begin, dates_end = TimeTool.match_dates_from_name(path_list)
    dates = TimeTool.get_average_dates(dates_begin, dates_end)

    shc = sagea.SHC.io.from_gfc(path_list, lmax=lmax, key="GRCOF2")

    # Low-degree replacement
    from sagea.sgio import read_low_degs
    c20c30 = read_low_degs(path_TN14, dates)
    for low_id in ("c2,0", "c3,0"):
        shc.replace(low_id, c20c30[low_id], inplace=True)

    shc -= shc_gif48
    shc.value[:, :4] = 0       # zero out degree-0 and degree-1

    shc.filter.gaussian(300, inplace=True)
    shc.convert(from_type="Geopotential", to_type="EWH", inplace=True)
    grid_list.append(shc.synthesize.to_grid(grid_space=1))

# ── 3. TCH Error-II assessment ────────────────────────────────────────────────
import cartopy.crs as ccrs

grid_err_tch = tch(*grid_list, mode=TCHMode.OLS)
for label, g_err in zip(["CSR", "GFZ", "JPL"], grid_err_tch):
    g_err.value *= 100     # m → cm
    g_err.plot(vmin=0, vmax=6, projection=ccrs.Robinson(), cmap="jet")

# ── 4. TCA Error-II assessment ────────────────────────────────────────────────
grid_err_tca = tca(*grid_list, mode=TCAMode.CLASSIC)
for label, g_err in zip(["CSR", "GFZ", "JPL"], grid_err_tca):
    g_err.value *= 100
    g_err.plot(vmin=0, vmax=6, projection=ccrs.Robinson(), cmap="jet")
```
